...
class Application extends React.Component {
  ...
  fetchFileList() {
    get("/api/files").end((err, res) => {
      this.setState({
        files: res.body.map((path) => {
          return {path, name: last(path.split("/"))};
        }),
  ...
  render() {
    ...
    return (
      <div>
        <input type="text" value={filter} onChange={this.updateFilter}/>
        <File file={fileTree.root} renderFile={this.renderFile} isOpen={isOpen}/>
      </div>
    );
  }
}
